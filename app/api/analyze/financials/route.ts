...
async function extractTextFromPDF(arrayBuffer: ArrayBuffer): Promise<string> {
  try {
    ...
    throw new Error("Unable to extract text from PDF. Please try converting to TXT or CSV format.")
  }
}
...
async function extractTextFromFile(file: File): Promise<string> {
  ...
  try {
    if (fileType === "application/pdf" || fileName.endsWith(".pdf")) {
      ...
      throw new Error(`Unsupported file type: ${fileType}. Please upload PDF, CSV, or TXT files.`)
    }
  } catch (error) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    const analysisResponse = await callNvidiaAPI(
      [{ role: "user", content: `Financial data to analyze:\n\n${limitedText}` }],
      systemPrompt,
    )
    ...
    try {
      ...
      analysisResult = {
        businessType: hasNumbers ? "Small to Medium Business" : "Business Entity",
        revenueTrend: hasRevenue ? "Revenue data present in document" : "Revenue information not clearly identified",
        largestCostCenters: hasCosts ? "Cost information found in document" : "Cost breakdown not clearly visible",
        profitMargins: hasNumbers ? "Financial metrics present" : "Profit margin data not clearly identifiable",
